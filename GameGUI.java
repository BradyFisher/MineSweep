...
public class GameGUI extends JFrame implements ActionListener {
	...
	private static final int LABEL_TOP = 850;
	/** x coord of the "n unmarked mines remain" label. */
	private static final int LABEL_LEFT = 400;
	...
	private void initDisplay()	{
		...
		lossMsg = new JLabel();
		lossMsg.setBounds(LABEL_LEFT, LABEL_TOP + LABEL_HEIGHT_INC, 400, 30);
	...
	private void signalLoss() {
		...
		for (int k = 0; k < board.size(); k++){
			if(board.MTileAt(k/board.get_num_rows(),k % board.get_num_columns()).get_mine()){
				board.MTileAt(k/board.get_num_rows(),k % board.get_num_columns()).set_status('M');
			}
		}
		repaint();
		totalGames++;
